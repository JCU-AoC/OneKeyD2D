...
public:
    Rotate2D(float radian = 0, const Vector::Vec2& point = Vector::Vec2())
        : m_Data(point.x, point.y, radian)
    {
    }
...
namespace RoomObj {
    ...
    public:
        ...
        /// <param name="obj"></param>
        /// <returns></returns>
        auto FindObj(RoomObject* obj)
        {
            return m_ObjectSet.find(obj);
        }
        /// <summary>
        /// 添加对象到集合中
        /// 添加成功返回当前集合数量
        ...
        /// <param name="obj"></param>
        /// <returns></returns>
        long long AddObject(RoomObject* obj)
        {
            if (!obj)
                return -1;
    ...
    public:
        ...
        Vector::Vec2 GetShowPosition()const
        {
            return Vector::Vec2(m_ShowRectangle.left, m_ShowRectangle.top);
        }
        /// <summary>
        /// 显示区域的宽度与高度
        /// </summary>
    ...
    public:
        ...
        static Vector::Vec2 ScenceToCamera(const Vector::Vec2& pos, const Camera2D& camera)
        {
            ...
            result = Vector::Vec2((float)result.Dot(right), (float)result.Dot(up.Normalize()));
            result.y *= -1;
            return result;
        }
        /// <summary>
    ...
    public:
        ...
        void Init(
            Game::MainWind_D2D* wind,
            const std::wstring& ShowText,
            WindCallback::ButtonCallback callBack,
            const D2D1_COLOR_F& TextColor = D2D1::ColorF(0),
            const D2D1_COLOR_F& bkColor = D2D1::ColorF(1, 1, 1),
            void* UserData = 0)
        {
            m_Button.Init(0, 0, 0, 0, ShowText, TextColor, bkColor, callBack, wind, (long long)UserData);
        }
    ...
    public:
        ...
        void Draw(Game::MainWind_D2D* wind, const Camera2D& Camera) override
        {
            ...
            for (int x = (int)leftBottom.x; x < (int)rightTop.x + 2; ++x) {
                for (int y = (int)rightTop.y + 1; y > (int)leftBottom.y - 1; --y) {
    ...
    public:
        ...
        /// 设置动画对应的数据
        /// 会根据对应的动画长度生成默认的顺序索引，
        /// 如果要设置指定顺序，使用SetIndex函数
        /// </summary>
        /// <param name="data">目标动画控件</param>
        void SetData(WindControl::d2dPictureAnimationBase* data)
        {
            if(data)
            {
                m_AnimationData = data;
                auto size = m_AnimationData->GetAnimationSize();
                m_Index.clear();
        ...
        void Draw(Game::MainWind_D2D* wind, const Camera2D& Camera) override
        {
            if (!wind || (!m_AnimationData) || m_Index.empty())
                return;
    ...
    class KeyframesAnimation : public RoomScence {
        struct Keyframe {
            D2D1::Matrix3x2F mat;
            float runTime;
            float opacity = 1.0f;
            Keyframe(D2D1::Matrix3x2F m, float rt, float o)
                : mat(m)
                , runTime(rt)
                , opacity(o)
            {
        ...
        RoomObject* m_Aim = nullptr;
        std::vector<Keyframe> m_Keyframes;
        Keyframe* m_CurrentKeyframe = nullptr;

    public:
        KeyframesAnimation(RoomObject* aim = nullptr)
            : m_Aim(aim)
        {
        }

        void AddKeyframe(D2D1::Matrix3x2F transform, float runTime, float opacity = 1.f, long long pos = -1)
        {
            ...
            m_Keyframes.insert(m_Keyframes.begin() + pos, Keyframe(transform, runTime, opacity));
        }
        void DeleteKeyframe(long long pos = -1)
        {
            auto size = m_Keyframes.size();
            if (pos < 0) {
                pos += size + 1;
